...
4. **QR tree (Contact)** — a dependency-free canvas engine mirroring tree.icqr.com: a procedural recursive tree whose leaves are the 436 dark modules of a real QR (github.com/sayon999-d); tap and the leaves fold flat into a scannable grid, tap again and the tree grows back. Keyboard accessible (Enter/Space), respects reduced motion, renders only while on screen
